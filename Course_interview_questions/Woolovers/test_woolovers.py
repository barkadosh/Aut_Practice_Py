...
class TestWooLoversWeb:
    ...
    def test_verify_price(self):
        try:
            ...
            time.sleep(3)
            driver.find_elements(By.XPATH, "//div[text()='Done']")[15].click()
            products_prices = driver.find_elements(By.CSS_SELECTOR, "div.container.products-container span.pricing__price.pricing__price--new:last-of-type")
            prices = []
            for i in range(0, len(products_prices)):
                price = driver.find_elements(By.CSS_SELECTOR,
                                                           "div.container.products-container span.pricing__price.pricing__price--new:last-of-type")[i].text
                price = price.replace("\u200c", "")
                price = price.replace("$", "")
                if float(price) >= 151:
                    pytest.fail()
